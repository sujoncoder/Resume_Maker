            ...
            <div className="container mx-auto flex justify-between items-center">
                ...
                <div className="hidden sm:flex space-x-4 items-center">
                    {links.map((link) => (
                        ...
                        >
                            <p className='text-lg font-medium hover:text-blue-400 duration-300 md:flex-none md:justify-start md:p-2 md:px-3'>{link.name}</p>
                        </Link>
                    ))}
                ...
                {isMobileMenuOpen && (
                    <div className="w-full h-full absolute top-16 text-xl font-semibold px-20 bg-white p-2 space-y-2 right-2">
                        {links.map((link) => (
                            <Link key={link.name} href={link.href}>
                                <p className='text-lg text-center border-b-2 py-2 rounded hover:text-blue-500 duration-300'>{link.name}</p>
                            </Link>
                        ))}
